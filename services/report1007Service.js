...
const Report1007 = require('../models/report1007Model');
...
exports.generateReport = async (req, res) => {
  try {
    ...
    objectReportResume.code = '1007GR';
    ...
    await Report1007.collection.deleteMany({
      companyId: userInfo.companyId
    });
    let arrayGenerated = [];
    ...
    for await (const reportData of masterReportData) {
      ...
      objectGenerated.ingresosBrutos = reportData.balanceAmountCompanyCurrency;
    ...
    for await (const suscriberData of suscriberDebtData) {
      ...
      objectGenerated.ingresosBrutos = suscriberData.value;
    ...
    nroCedulasUnicos.forEach(function(doc) {
      ...
      conceptosUnicos.forEach(function(concepto) {
        ...
        if (byConceptosList && byConceptosList.length > 0) {
          ...
          objectGenerated.companyId = userInfo.companyId;
          objectGenerated.userId = userInfo._id;
          objectGenerated.ingresosBrutos = 0;
          byConceptosList.forEach(function(docSum) {
            ...
            let ingresosBrutosDef = 0;
            if (getNum(docSum.ingresosBrutos)) {
              ingresosBrutosDef =
                getNum(objectGenerated.ingresosBrutos) +
                getNum(docSum.ingresosBrutos);
              objectGenerated.ingresosBrutos = ingresosBrutosDef;
            }
          });
          arrayGenerated.push(objectGenerated);
        }
    ...
    arrayGeneratedTmp.forEach(function(rowFinal) {
      const ingresosBrutosTmp = getNum(rowFinal.ingresosBrutos);
      const signus = Math.sign(ingresosBrutosTmp);
      let flag = false;
      if (ingresosBrutosTmp === 0) {
        flag = true;
      } else {
        if (signus === 1) {
          if (ingresosBrutosTmp < 500000) {
            flag = true;
          } else {
            flag = false;
          }
          ...
          if (signus === -1) {
            const ingresosBrutosTmpPositive = ingresosBrutosTmp * -1;
            if (ingresosBrutosTmpPositive < 500000) {
              flag = true;
            } else {
              flag = false;
            }
      ...
      if (flag) {
        let ingresosBrutosDef = 0;
        ingresosBrutosDef =
          getNum(objectGenerated.ingresosBrutos) + ingresosBrutosTmp;
        objectGenerated.ingresosBrutos = ingresosBrutosDef;
      } else {
        arrayGenerated.push(rowFinal);
      }
    });
    if (objectGenerated.ingresosBrutos !== 0) {
      arrayGenerated.push(objectGenerated);
    }
    ...
    arrayGeneratedTmp.forEach(row => {
      if (row.nroIdentificacion === '222222222') {
        const ingresosBrutosTmp = getNum(row.ingresosBrutos);
        let ingresosBrutosDef = 0;
        ingresosBrutosDef =
          getNum(objectGenerated.ingresosBrutos) + ingresosBrutosTmp;
        objectGenerated.ingresosBrutos = ingresosBrutosDef;
...
exports.sendReportCSV = async (req, res) => {
  try {
    ...
    const reportData = await Report1007.find({
      companyId: userInfo.companyId
    }).lean();
