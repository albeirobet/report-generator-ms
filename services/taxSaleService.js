...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 0) {
          ...
          if (fileTitle !== constants.TAX_SALE_TEMPLATE_TITLE) {
        ...
        if (count > constants.TAX_SALE_TEMPLATE_ROW_INIT) {
          const taxSale = {
            identificationType: currRow.getCell(2).value,
            identificationNumber: currRow.getCell(3).value,
            name: currRow.getCell(4).value,
            tax: currRow.getCell(5).value,
            companyId: userInfo.companyId,
            userId: userInfo._id
          };
          taxSales.push(taxSale);
        }
        count += 1;
      });
    });
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    await TaxSale.collection
      .insertMany(taxSales)
      .then(function() {
        summaryLoadedData.message =
          reportGeneratorMessages.M_REPORT_GENERATOR_MS_01;
        summaryLoadedData.counter = taxSales.length;
        console.log('Insert Data Finish');
        async function finishReport() {
          // Actualizando información encabezado reporte
          objectReportResume.state = 'uploaded_data';
          objectReportResume.percentageCompletition = 100;
          objectReportResume.counterRows = taxSales.length;
...
exports.delete = async (req, res) => {
  try {
    const userInfo = await userService.getUserInfo(req, res);
    await TaxSale.deleteMany({ companyId: userInfo.companyId });
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'TXSTM';
    objectReportResume.companyId = userInfo.companyId;
...
exports.count = async (req, res) => {
  ...
  try {
    const userInfo = await userService.getUserInfo(req, res);
    return await TaxSale.countDocuments({
      companyId: userInfo.companyId
    });
  } catch (err) {
    console.log(err);
...
exports.get = async (req, res) => {
  ...
  const data = await TaxSale.findById(req.params.id);
...
exports.getAll = async (req, res) => {
  ...
  const dataTable = new APIFeatures(TaxSale.find(), req.query)
    .filter(userInfo.companyId, false, filterColumns)
    .sort()
    .limitFields()
    .paginate();
  const counter = new APIFeatures(TaxSale.find(), req.query).filter(
    userInfo.companyId,
    true,
    filterColumns
  );
