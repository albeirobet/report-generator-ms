  ...
  try {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count > constants.MASTER_REPORT_TEMPLATE_ROW_INIT) {
          const report = {
            ...
            originalDocumentDate: currRow.getCell(8).value,

            accountingSeatType: currRow.getCell(9).value,
            accountingSeatAnnulled: currRow.getCell(10).value,
            originalDocumentAnnulledId: currRow.getCell(11).value,
            accountingSeatAnnulment: currRow.getCell(12).value,
            extraOriginalDocumentAnulledId: currRow.getCell(13).value,
            extraOriginalDocumentId: currRow.getCell(14).value,
            originalDocumentPosition: currRow.getCell(15).value,

            thirdIDExtra: currRow.getCell(16).value,
            thirdNameExtra: currRow.getCell(17).value,

            originalPosition: currRow.getCell(18).value,
            operatingDocumentID: currRow.getCell(19).value,
            operatingDocumentCounterpartID: currRow.getCell(20).value,
            thirdId: currRow.getCell(21).value,
            ...
            balanceAmountCompanyCurrency: currRow.getCell(27).value,
            companyId: userInfo.companyId,
            userId: userInfo._id
          };
    ...
    await MasterReport.collection
      ...
      .then(function() {
        ...
        summaryLoadedData.counter = masterReport.length + countDB;
        console.log('Insert Data Finish');
        async function finishReport() {
          // Actualizando información encabezado reporte
          objectReportResume.state = 'uploaded_data';
          objectReportResume.percentageCompletition = 100;
          objectReportResume.counterRows = masterReport.length + countDB;
