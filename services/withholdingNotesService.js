...
const WithholdingNotes = require('../models/withholdingNotesModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 0) {
          ...
          if (fileTitle !== constants.WITHHOLDING_NOTES_TEMPLATE_TITLE) {
        ...
        if (count > constants.WITHHOLDING_NOTES_TEMPLATE_ROW_INIT) {
          const withholdingnote = {
            ...
            confirmedInboundDeliveryID: currRow.getCell(7).value,
            purchaseOrderID: currRow.getCell(8).value,
            refund: currRow.getCell(9).value,
            extraSupplierID: currRow.getCell(10).value,
            extraSupplierName: currRow.getCell(11).value,
          ...
          withholdingNotes.push(withholdingnote);
        }
        count += 1;
      });
    });
    ...
    await reportFunctionsUpdate.updateReportUploader(objectReportResume);
    await WithholdingNotes.collection
      .insertMany(withholdingNotes)
      .then(function() {
        summaryLoadedData.message =
          reportGeneratorMessages.M_REPORT_GENERATOR_MS_01;
        summaryLoadedData.counter = withholdingNotes.length;
        console.log('Insert Data Finish');
        async function finishReport() {
          // Actualizando información encabezado reporte
          objectReportResume.state = 'uploaded_data';
          objectReportResume.percentageCompletition = 100;
          objectReportResume.counterRows = withholdingNotes.length;
...
exports.delete = async (req, res) => {
  try {
    const userInfo = await userService.getUserInfo(req, res);
    await WithholdingNotes.deleteMany({ companyId: userInfo.companyId });
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'NRETM';
    objectReportResume.companyId = userInfo.companyId;
...
exports.count = async (req, res) => {
  ...
  try {
    const userInfo = await userService.getUserInfo(req, res);
    return await WithholdingNotes.countDocuments({
      companyId: userInfo.companyId
    });
  } catch (err) {
    console.log(err);
...
exports.get = async (req, res) => {
  ...
  const data = await WithholdingNotes.findById(req.params.id);
...
exports.getAll = async (req, res) => {
  ...
  const dataTable = new APIFeatures(WithholdingNotes.find(), req.query)
    .filter(userInfo.companyId, false, filterColumns)
    .sort()
    .limitFields()
    .paginate();
  const counter = new APIFeatures(WithholdingNotes.find(), req.query).filter(
    userInfo.companyId,
    true,
    filterColumns
  );
