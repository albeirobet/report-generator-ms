...
const ChartAccountModel = require('../models/chartAccountModel');
...
exports.loadAsyncy = async (req, res) => {
  try {
    ...
    await workbook.xlsx.readFile(pathx).then(function() {
      ...
      workSheet.eachRow(function(row, rowNumber) {
        ...
        if (count === 0) {
          ...
          if (fileTitle !== constants.CHART_ACCOUNT_TEMPLATE_TITLE) {
        ...
        if (count > constants.CHART_ACCOUNT_TEMPLATE_ROW_INIT) {
          const chartAccount = {
            accountID: currRow.getCell(2).value,
            accountDescription: currRow.getCell(3).value,
            accountType: currRow.getCell(4).value,
            format: currRow.getCell(5).value,
            concept: currRow.getCell(6).value,
            extraFormat: currRow.getCell(7).value,
            extraConcept: currRow.getCell(8).value,
            deductible: currRow.getCell(9).value,
            noDeductible: currRow.getCell(10).value,
            ivaDeductible: currRow.getCell(11).value,
            ivaNoDeductible: currRow.getCell(12).value,
            companyId: userInfo.companyId,
            userId: userInfo._id
          };
          chartAccounts.push(chartAccount);
        }
        count += 1;
      });
    });
    ...
    await ChartAccountModel.collection
      .insertMany(chartAccounts)
      .then(function() {
        summaryLoadedData.message =
          reportGeneratorMessages.M_REPORT_GENERATOR_MS_01;
        summaryLoadedData.counter = chartAccounts.length + countDB;
        console.log('Insert Data Finish');
        async function finishReport() {
          // Actualizando información encabezado reporte
          objectReportResume.state = 'uploaded_data';
          objectReportResume.percentageCompletition = 100;
          objectReportResume.counterRows = chartAccounts.length + countDB;
...
exports.delete = async (req, res) => {
  try {
    const userInfo = await userService.getUserInfo(req, res);
    await ChartAccountModel.deleteMany({ companyId: userInfo.companyId });
    // Defino objeto y variables estandar para el resumen de la carga
    const objectReportResume = {};
    objectReportResume.code = 'PCUTM';
    objectReportResume.companyId = userInfo.companyId;
...
exports.count = async (req, res) => {
  ...
  try {
    const userInfo = await userService.getUserInfo(req, res);
    return await ChartAccountModel.countDocuments({
      companyId: userInfo.companyId
    });
  } catch (err) {
    console.log(err);
...
exports.get = async (req, res) => {
  ...
  const data = await ChartAccountModel.findById(req.params.id);
...
exports.getAll = async (req, res) => {
  ...
  const dataTable = new APIFeatures(ChartAccountModel.find(), req.query)
    .filter(userInfo.companyId, false, filterColumns)
    .sort()
    .limitFields()
    .paginate();
  const counter = new APIFeatures(ChartAccountModel.find(), req.query).filter(
    userInfo.companyId,
    true,
    filterColumns
  );
